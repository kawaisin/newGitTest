...
<?php if ( have_posts() ) : ?>
	...
	// Otherwise if we have a featured image, try to get text from the image
		...
		$title = sprintf( __( 'Day: %s' ), '<span>' . get_the_date() . '</span>' );

	} elseif ( is_month() ) {
		$title = sprintf( __( 'Month: %s' ), '<span>' . get_the_date( 'F Y' ) . '</span>' );

	} elseif ( is_year() ) {
		$title = sprintf( __( 'Year: %s' ), '<span>' . get_the_date( 'Y' ) . '</span>' );
	...
	if ( $image_url ) :
		...
		if ( is_front_page() ) {
			...
			$overlay_class = 'cover-image-overlay-inside';
		} else {
			$image_class = 'section-image';
			$overlay_class = 'section-image-overlay-inside';
		}
		...
		<header class="content-header-image">
			<div class="<?php echo $image_class; ?>" style="background-image: url('<?php echo $image_url; ?>')">
				...
				?>
				<?php				
				// Only for blog page, show a scroll down icon
				if ( is_home() ) {
					echo '<p>This is the Blog</p>';
				}
				?>
